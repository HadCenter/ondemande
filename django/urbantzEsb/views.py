...
def SendFromFileToUrbantzAsTasks(request):
	...
	for row in excelfile.itertuples() :
		...
		buildingInfo = BuildingInfoUrbantz(interphoneCode= row.CODE_interphone , floor=row.Etage,digicode2=row.Tel2 , hasElevator= hasElevator , hasInterphone = hasInterphone)
		contact = ContactUrbantz(name = row.Nom_destinataire,phone = row.Telephone ,email= row.email,buildingInfo = buildingInfo)
		metadata = Metadata(CodeArticle= row.CodeArticle , Porte =row.Porte , Instructions = row.Instructions,Tel3= row.Tel3)
		...
		items=[]
		items.append(item)
		task = UrbantzTask(taskId=row.RefClient,client=row.Expediteur,type = typeService, date= row.Date_livraison , taskReference=row.Ref_commande, contact = contact,metadata = metadata, address = location , timeWindow = timewindow, timeWindow2= timewindow ,items = items)
