...
export class StatusJobsComponent implements OnInit {
  ...
  getPlans() {
    this.service.getAllFacturationPlans().subscribe(
      (res) => {
  ...
  checkWhichJobCanBeExecuted() {
    ...
    this.advancedTable.forEach((job) => {
      ...
      if (job.status != "Terminée") {
        this.advancedTable.map((element) =>
          element != job
            ? (element.canBeExecuted = false)
            : (element.canBeExecuted = true)
        );
      }
      //enable executing step 4 and 5 if step3 was not executed today
      if (
        job.plan == "step 3" &&
        job.derniere_execution.toString().substring(0, 10) !=
          this.datePipe.transform(new Date(), "yyyy-MM-dd")
      ) {
        this.advancedTable.map((element) =>
          element != job ? (element.canBeExecuted = false) : null
        );
      }
    });
  }
  ...
  executeJob(planData) {
    if (planData.plan == "step 4") {
      ...
      this.service.launchPlan(data).subscribe(
        (res) => {
          this.actualiser();
        },
        (err) => {}
  ...
  goToPage(row){
    if (row.plan.includes("4")) {
      this.goToBilling();
    } else if (row.plan.includes("5")) {
      this.goToSalesForcePage();
    }
  }
  verify(row) {
    if (row.plan.includes("4")) {
  ...
  verifyStep5(row) {
    ...
    console.log(row);
    var data = {
      "date":this.advancedTable[0].derniere_execution.substring(0, 10)

    }
    this.service.checkBillingFileInSalesforce(data).subscribe(res =>{
      if(res == "Le montant de la facture a été mise à jour avec succès" || res == "Le montant de la facture est valide dans Salesforce"){
        this.openSnackBar("Fichier vérifié avec succès.","Ok",10000);
        row.verified = "validate";
      }else{
        this.openSnackBar("Fichier introuvable","Ok",10000);
        row.verified = "not validate";
      }
    },
    err=> {
      console.error(err);
      if(err.error == "facture non trouvé dans Salesforce"){
        row.verified = "not validate";
        this.openSnackBar("Facture non trouvé dans Salesforce","Ok",10000);
      }else if(err.error == "Introuvable") {
        row.verified = "not validate";
        this.openSnackBar("Fichier billing introuvable","Ok",10000);
      }else{
  ...
  verifyStep3(row) {
    this.openSnackBar("Vérification du fichier en cours...", "Ok", 4000);
    this.service.checkFileMAD().subscribe(
      (res) => {
        if (res == "file found") {
          this.openSnackBar("Fichier vérifié avec succès.", "Ok", 10000);
          row.verified = "validate";
          ...
          this.openSnackBar(
            "Fichier introuvable, re-exécuter le step 3!",
            "Ok",
            10000
          );
          row.verified = "not validate";
  ...
  verifyStep4(row) {
    ...
    const file =
      "billing" +
      this.advancedTable[0].derniere_execution.substring(0, 10) +
      ".xlsx";
    var data = {
      file: file,
    };
