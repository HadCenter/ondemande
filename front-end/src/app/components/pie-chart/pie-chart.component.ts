...
export class PieChartComponent extends BasePieChartComponent implements OnInit {
  ...
  public ngOnInit() {
    ...
    const animatedData = [
      ...rawData.map(job => ({ key: job.key, end: job.nbr, y: 0 })),
      {
        key: 'Pending',
        y: 23.9,
      },
    ...
    nv.addGraph(() => {
      ...
      pieChart.tooltip.enabled(true)
        ...
        .contentGenerator((d) => {
          ...
          return `${d.animatedData.y} commandes`;
        });
      ...
      d3.select(this.el.nativeElement.querySelector('.nv-pie .nv-pie'))
        ...
        .attr('width', '30')
        .attr('height', '30')
        .attr('xlink:href', 'assets/images/shopping_basket-white.svg')
